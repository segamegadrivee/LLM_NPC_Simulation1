...
public class ContextRetriever : MonoBehaviour
{
        ...
        List<WorldEvent> relevantEvents)
    {
        ...
        evaluation.appearanceBlockReason = GetAppearanceBlockReason(entry, playerState, playerMessage);
    ...
    // Appearance categories. An entry whose appearance tags fall within EXACTLY ONE category is
    // treated as a single "appearance signal" entry (e.g. public_guard_armor_signal). Entries that
    // span several categories (e.g. mira_reads_people_in_tavern, which carries both armor and cloak)
    // are broad disposition knowledge and are NOT gated by appearance.
    ...
    private static string GetAppearanceBlockReason(KnowledgeEntry entry, PlayerState playerState, string playerMessage)
    {
        ...
        bool armor = HasAnyTag(entry.tags, AppearanceArmorTags);
        bool cloak = HasAnyTag(entry.tags, AppearanceCloakTags);
        bool weapon = HasAnyTag(entry.tags, AppearanceWeaponTags);
        ...
        if (HasAnyTag(visibleTags, category))
        {
    ...
    private static void AddVisibleToken(List<string> result, string value)
    {
        ...
        if (!ContainsIgnoreCase(result, token))
        {
            result.Add(token);
        }
    }

    private static bool HasAnyTag(List<string> tags, string[] set)
    {
